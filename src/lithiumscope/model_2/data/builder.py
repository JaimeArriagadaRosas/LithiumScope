...
def _sentinel_config(config: dict) -> SentinelConfig:
    ...
    return SentinelConfig(
        ...
        request_timeout_seconds=float(imagery.get("request_timeout_seconds", 60)),
        max_retries=int(imagery.get("max_retries", 3)),
        scene_cache_decimals=int(imagery.get("scene_cache_decimals", 4)),
...
def _sample_key(sample_id: str, latitude: float, longitude: float) -> str:
...
def _load_valid_manifest(path: Path) -> pd.DataFrame:
    ...
    try:
        ...
        logger.warning("Could not read Model 2 checkpoint %s; ignoring it", path, exc_info=True)
        return pd.DataFrame(columns=_MANIFEST_COLUMNS)

    if "image_path" not in frame.columns:
        return pd.DataFrame(columns=_MANIFEST_COLUMNS)
    valid = frame[frame["image_path"].map(lambda value: Path(str(value)).exists())].copy()
...
def _persist_progress(
    ...
    manifest = pd.DataFrame(list(rows.values()), columns=_MANIFEST_COLUMNS)
...
def _progress_message(done: int, total: int, ready: int, failed: int, resumed: int) -> str:
    return (
        f"    progreso {done}/{total} | listos={ready} | "
        f"omitidos={failed} | reanudados={resumed}"
    )
...
def ensure_model_2_dataset(model_1_dataset: Path, force: bool = False) -> Model2DatasetResult:
    ...
    partial_manifest_path = _resolve_project_path(
        str(training.get("partial_manifest_path", "data/processed/model_2/training_manifest.partial.csv"))
    )
    failures_path = _resolve_project_path(
        str(training.get("failures_path", "data/processed/model_2/training_failures.csv"))
    )
    image_cache = _resolve_project_path(str(training["image_cache_dir"]))
    minimum_samples = int(training.get("minimum_samples", 50))
    checkpoint_every = max(1, int(imagery.get("checkpoint_every", 10)))
    progress_every = max(1, int(imagery.get("progress_every", 10)))
    max_consecutive_failures = max(1, int(imagery.get("max_consecutive_sample_failures", 10)))
    ...
    provider = Sentinel2Provider(sentinel)
    ...
    for row in checkpoint.to_dict(orient="records"):
        key = _sample_key(str(row["sample_id"]), float(row["latitude"]), float(row["longitude"]))
        rows[key] = row
    ...
    print(f"  Cache recuperada: {resumed_samples} muestras")
    print("  Validando proveedor STAC antes de iniciar la adquisición...")

    # Provider-wide configuration errors must stop before hundreds of identical requests.
    provider.validate()
    print("  Proveedor STAC: OK")
    ...
    try:
        for position, sample in samples.iterrows():
            ...
            key = _sample_key(sample_id, latitude, longitude)

            if key in rows:
                if sample_number % progress_every == 0 or sample_number == len(samples):
                    print(
                        _progress_message(
                            sample_number,
                            len(samples),
            ...
            try:
                ...
                logger.exception("Fatal Sentinel provider error; aborting acquisition")
                raise
                ...
                logger.info("No Sentinel scene for sample=%s: %s", sample_id, exc)
                ...
                logger.warning("Sentinel asset failed sample=%s: %s", sample_id, exc)
                ...
                logger.exception("Unexpected Sentinel acquisition failure sample=%s", sample_id)

            if sample_number % checkpoint_every == 0:
                _persist_progress(rows, failures, partial_manifest_path, failures_path)
            ...
            if consecutive_failures >= max_consecutive_failures:
                raise RuntimeError(
                    ...
                    f"{consecutive_failures} fallos de asset inesperados consecutivos. "
                    "Se detiene para evitar repetir cientos de errores. Revise training_failures.csv."
                )
    finally:
        _persist_progress(rows, failures, partial_manifest_path, failures_path)
    ...
    if len(manifest) < minimum_samples:
        raise RuntimeError(
            ...
            f"Disponibles={len(manifest)}, mínimo={minimum_samples}, fallos={failed_samples}. "
            f"Checkpoint={partial_manifest_path}. Revise {failures_path}."
        )

    _atomic_write_csv(manifest, manifest_path)
    logger.info(
        "Model 2 manifest generated automatically: %s ready=%d failed=%d resumed=%d",
        manifest_path,
        len(manifest),
        failed_samples,
        resumed_samples,
