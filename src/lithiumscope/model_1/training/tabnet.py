...
def _log_warnings(records) -> None:
    for record in records:
        ...
        logger.warning("TabNet warning captured once: %s", message)
...
class TabNetRegressorAdapter(BaseEstimator, RegressorMixin):
    ...
    def _new_model(self):
        try:
            ...
            raise RuntimeError(
                "TabNet is optional. Install with: pip install -e '.[ml]'"
            ) from exc
    ...
    def _fit_with_internal_validation(self, x, y):
        if len(x) < 20 or not 0.0 < self.validation_fraction < 0.5:
        ...
        with warnings.catch_warnings(record=True) as captured:
            ...
            model.fit(
                ...
                virtual_batch_size=min(128, max(16, len(x_train) // 4)),
            )
        _log_warnings(captured)
        best_epoch = int(getattr(model, "best_epoch", self.max_epochs - 1)) + 1
        return model, max(1, min(best_epoch, self.max_epochs))

    def fit(self, x, y):
        x = np.asarray(x, dtype=np.float32)
        y = np.asarray(y, dtype=np.float32).reshape(-1, 1)

        model, best_epoch = self._fit_with_internal_validation(x, y)
