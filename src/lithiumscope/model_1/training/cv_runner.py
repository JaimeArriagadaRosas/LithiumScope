...
def run_nested_cv(prepared, algorithm: str, device: DeviceInfo, config: dict) -> CVRunResult:
    ...
    trials = int(optimization.get("per_algorithm_trials", {}).get(algorithm, optimization.get("n_trials", 20)))
    ...
    for fold, (train_idx, test_idx) in enumerate(outer.split(prepared.x), start=1):
        ...
        if optimize:
            ...
            params = optimize_with_optuna(factory, spec.module.optuna_space, x_train, y_train, inner, trials)
        ...
        folds.append({"fold": fold, **metrics, "n_train": len(train_idx), "n_test": len(test_idx)})
    ...
    if optimize:
        def final_factory(candidate: dict):
            return build_pipeline(algorithm, device, seed, prepared.schema, candidate)
        final_params = optimize_with_optuna(final_factory, spec.module.optuna_space, prepared.x, prepared.y, inner, trials)

    elapsed = time.perf_counter() - started
    return CVRunResult(algorithm, spec.label, predictions, fold_table, overall, elapsed, final_params)
