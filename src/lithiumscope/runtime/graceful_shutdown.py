...
class GracefulShutdownManager:
    def __init__(
        self,
        child_timeout_seconds: float = 2.0,
    ) -> None:
    ...
    def request_shutdown(
        ...
        if reason != "normal":
            print(
                "\nCerrando LithiumScope "
                f"de forma segura ({reason})..."
            )
    ...
    def finalize(self) -> None:
        ...
        self._stop_children()
        logger.info(
            "LithiumScope runtime finalized"
        )
        finalize_logging()
