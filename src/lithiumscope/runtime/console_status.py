...
class ConsoleStatusManager:
    """Coordinates one rewriteable status line with normal console messages."""
    ...
    def rewrite(
        ...
        with self._lock:
            if self._active is not spinner:
                return
            spinner._rewrite_unlocked(
                self._truncate(text)
            )
    ...
    def message(self, text: str) -> None:
        with self._lock:
            ...
            self._stream.flush()
            if active is not None:
                active._rewrite_unlocked(
                    self._truncate(active._display_text())
                )
...
class Spinner:
    """Single-line spinner with bounded refresh and coordinated rewrites."""
    ...
    def __init__(
        ...
        *,
        ...
        self._manager = (
            ...
            else ConsoleStatusManager(self._stream)
        )
