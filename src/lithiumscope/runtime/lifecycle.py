...
from lithiumscope.runtime.graceful_shutdown import (
    GracefulExit,
...
def _print_session_summary() -> None:
    ...
    print(
        f"  Warnings           "
        f"{warnings_info['unique']} únicos / "
        f"{warnings_info['total']} total"
    )
    print(
    ...
    print(
        f"  Log de sesión      "
        f"{summary['session_log']}"
    )
    if summary["error_log_exists"]:
        print(
            f"  Log de errores     "
            f"{summary['error_log']}"
        )
...
def run_application(
    ...
    try:
        ...
        if recovered:
            logger.warning(
                "Recovered %d abandoned training "
                "run(s) as crashed: %s",
                len(recovered),
                ", ".join(
                    path.name
        ...
        logger.info(
            "Matplotlib backend initialized: %s",
            backend,
        )
        ...
        if not report.core_ready:
            ...
            print(
                "\nPreboot falló en requisitos "
                "esenciales. Revise el reporte "
                "indicado arriba."
            )
            return 2
